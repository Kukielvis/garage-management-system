...
@push('modals')
    ...
    <div id="payments-modal" class="modal fade payment" tabindex="-1" role="dialog" aria-labelledby="myPaymentLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myAppointmentModalLabel">Payment for Appointment</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="card-body">
                        <form id="payments_form" method="POST">
                            ...
                            <div class="form-group row">
                                <div class="col-md-4">
                                    <label for="type_id" class="mt-2">Phone Number: </label>
                                    <input id="phone_number" type="number" placeholder="254701234567">
                                </div>
                                <div class="col-md-4">
                                    <label for="category_id" class="mt-2">Amount: </label>
                                    <input id="amount_to_be_paid" type="text" disabled>
                                </div>
                            </div>
                            <button id="service_pricing_button" type="submit" class="btn btn-primary">
                                {{ __('Add') }}
...
@push('modal-scripts')
    <script>
                ...
                $.ajax({
                    ...
                    success: res=>{
                        ...
                        res.forEach(function (entry,index) {
                            ...
                            fill_data.push({id:entry['service_id'],text:entry['name'],amount:pricing[0]['amount']});
                        });
                        $('#service_id').select2({
                            multiple:'true',
                            data:fill_data,
        ...
        $(()=>{
            ...
            let data,url,amount = null
            let payments_modal = $('#payments-modal')
            ...
            appointments_form.on('submit',function (event) {
                ...
                data = {services:services,category_id:category_id,amount:amount};
                url = `{{ url('/add/request') }}`;

                ajaxPost(url,data);
                appointments_form[0].reset(0)
                type_select.trigger('change');
                requests_table.ajax.reload()
            });

            payments_modal.on('shown.bs.modal',function (event) {
                $('#amount_to_be_paid').val('KES'+amount)
            })
        });
    </script>
@endpush
